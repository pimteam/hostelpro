...
class HostelPROTextCaptcha {
	// verify the captcha
	static function verify($question, $answer) {
		$answer = stripslashes($answer);
		
		$captcha_questions = get_option('hostelpro_text_captcha');
	...
	static function generate() {
		...
		return trim($q)." <input type='text' name='hostelpro_text_captcha_answer'>\n<input type='hidden' name='hostelpro_text_captcha_question' value=\"".base64_encode(trim($q))."\">"; 
	}
}